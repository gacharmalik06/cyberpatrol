<?php   
    ...
    if(!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['new_password_retype'])){
        ...
        $check_password  = $bdd->prepare('SELECT password FROM users WHERE id = :id');
        ...
        if(password_verify($current_password, $data_password['password']))
        {
            ...
            if($new_password === $new_password_retype){
                ...
                // On redirige
                header('Location: ../landingClient.php?err=success_password');
                die();
            }
        }
        else{
            header('Location: ../landingClient.php?err=current_password');
            die();
        }
